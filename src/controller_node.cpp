...
ControllerNode::ControllerNode() : Node("clustering_node")
{
    ...
    if(this->filter && this->publishFilteredPc) {
        ...
        RCLCPP_INFO(this->get_logger(), "Created filtered point cloud publisher on topic: %s", this->filtered_topic.c_str());
    }
...
void ControllerNode::scanCallback(sensor_msgs::msg::PointCloud2::SharedPtr sub_cloud)
{
    ...
    if (this->filter)
    {
        ...
        // Check filtered bounds (stride is 4 floats per point due to alignment)
        cudaDeviceSynchronize();
        float minZ = 1e10, maxZ = -1e10;
        for(unsigned int i = 0; i < size; i++) {
            float z = partialOutput[i * 4 + 2];
            minZ = std::min(minZ, z);
            maxZ = std::max(maxZ, z);
        }
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("clustering_node"), "Publishing filtered PC with %d points", size);
            cudaDeviceSynchronize(); // Ensure GPU operations complete
            this->publishPc(partialOutput, size, filtered_cp_pub);
            RCLCPP_INFO(rclcpp::get_logger("clustering_node"), "Filtered PC published");
        }
