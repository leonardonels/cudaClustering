    ...
    unsigned int *out_num_points)
{
  ...
  if (!skip)
  {
    // reinterpret_cast float[4*nCount] in float4*
    auto input4 = thrust::device_pointer_cast(
        reinterpret_cast<float4 *>(inputData));
    auto output4 = thrust::device_pointer_cast(
    ...
    auto new_end = thrust::copy_if(
        ...
        output4,                 // inizio output
        [] __device__(int idx) { // operatore lambda per il filtro
          return idx == -1;
        });

    // calcolo numero di inlier copiati
    *out_num_points = new_end - output4;
